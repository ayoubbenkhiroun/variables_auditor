...
window.exportRulesConfig = function() {
  const config = {
    appName: "Paprika AuditFlow Pro",
    version: "2.0",
    exportDate: new Date().toISOString(),
    rules: rules,
    customReserved: Array.from(getCustomReserved()),
    sensitivityLevels: (document.getElementById('sensitivityLevels')?.value || '').split('\n').map(s => s.trim()).filter(Boolean),
    teamTags: (document.getElementById('teamTags')?.value || '').split('\n').map(t => t.trim()).filter(Boolean)
  };
  ...
  a.href = url;
  a.download = `Paprika_Audit_Rules_Config_${new Date().toISOString().slice(0, 10)}.json`;
...
window.importRulesConfig = function() {
  const input = document.getElementById('importRulesInput');
  if (input) input.click();
};

window.handleImportRulesFile = function(e) {
  ...
  reader.onload = function(evt) {
    try {
      ...
      if (!config || typeof config !== 'object') throw new Error("JSON invalide");

      if (config.rules && Array.isArray(config.rules)) {
        config.rules.forEach(importedRule => {
          if (!importedRule || typeof importedRule !== 'object') return;
          const r = rules.find(x => x.id === importedRule.id);
          if (r && typeof importedRule.enabled === 'boolean') {
            r.enabled = importedRule.enabled;
          }
        });
        if (typeof buildRulesGrid === 'function') buildRulesGrid();
      }
      if (config.customReserved && Array.isArray(config.customReserved)) {
        const el = document.getElementById('customReserved');
        if (el) el.value = config.customReserved.map(s => String(s || '').trim()).filter(Boolean).join('\n');
      }
      if (config.sensitivityLevels && Array.isArray(config.sensitivityLevels)) {
        const el = document.getElementById('sensitivityLevels');
        if (el) el.value = config.sensitivityLevels.map(s => String(s || '').trim()).filter(Boolean).join('\n');
      }
      if (config.teamTags && Array.isArray(config.teamTags)) {
        const el = document.getElementById('teamTags');
        if (el) el.value = config.teamTags.map(s => String(s || '').trim()).filter(Boolean).join('\n');
      }
      alert('Configuration des règles importée avec succès !');
    } catch(err) {
      alert('Erreur lors de la lecture du fichier JSON de configuration.');
    }
  };
